...
const Activities = () => {
    ...
    return (
        <div className="h-screen w-full flex flex-col p-8">
            <h1 className="text-center font-bold italic text-3xl text-primary-500">Actividades</h1>
                        ...
                        <tbody>
                            {users.map((user,index) => (
                                <td key={index} className="w-[329px] flex justify-start items-center pl-3 py-3">
                                    <input type="checkbox" className='mr-2 accent-primary-500' /><p className='flex items-center py-2'>{user.name} {user.lastName}</p>
                                </td>
                            ))}
                        </tbody>
                    </table>
